...
export async function downloadAndMaybeExtractAppAsync(url: string, platform: "ios" | "android", cachedAppPath?: string): Promise<string> {
...
export async function extractAppFromLocalArchiveAsync(appArchivePath: string, platform: "ios" | "android"): Promise<string> {
  const outputDir = path.join(getTmpDirectory(), uuidv4())
  await fs.promises.mkdir(outputDir, { recursive: true })
...
async function getAppPathAsync(outputDir: string, applicationExtension: string): Promise<string> {
  const appFilePaths = await glob(`./**/*.${applicationExtension}`, { cwd: outputDir, onlyFiles: false })
  if (appFilePaths.length === 0) throw Error("Did not find any installable apps inside tarball.")
...
async function tarExtractAsync(input: string, output: string): Promise<void> {
  try {
    if (process.platform !== "win32") {
      await spawnAsync("tar", ["-xf", input, "-C", output], { stdio: "inherit" })
      return
    }
  } catch (error: any) {
    console.warn(`Failed to extract tar using native tools, falling back on JS tar module. ${error.message}`)
  }
